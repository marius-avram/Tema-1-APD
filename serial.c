...
int main(int argc, char** argv){
	...
	while(index != iterations){
		market_info[index].resourceA = 0; 
		market_info[index].resourceB = 0; 
		...
		for(i=0; i<matrix_size; ++i){
			...
			resources[i][j] == 0 ? market_info[index].resourceA++: 
								   market_info[index].resourceB++;
			if (resources[i][j]==0 && prices[i][j]>market_info[index].maxA){
				market_info[index].maxA = prices[i][j];
			}
			else if (resources[i][j]==1 && prices[i][j]>market_info[index].maxB){
